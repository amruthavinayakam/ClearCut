import type { Actor, AppConfig, MonitorRecord, Project, WorkflowStatus } from "./types";
...
/** Subscribe to a project's progress stream. Returns an unsubscribe function. */
export function streamProject(
  ...
  source.onerror = () => {
    if (source.readyState === EventSource.CLOSED) {
      onFrame({ type: "error", phase: "failed", message: "Connection to the server was lost." });
    }
  };
