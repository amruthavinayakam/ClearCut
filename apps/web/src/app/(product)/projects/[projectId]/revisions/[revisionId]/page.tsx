...
import { apiRequest, getProject, isNotFound } from "@/lib/api-client";
...
export default async function RevisionPage({ params }: { params: Promise<{ projectId: string; revisionId: string }> }) {
  ...
  let revision;
  try {
    [project, revision] = await Promise.all([getProject(projectId), apiRequest(`/api/projects/${encodeURIComponent(projectId)}/revisions/${encodeURIComponent(revisionId)}`, ProjectRevisionSchema)]);
  } catch (error) {
