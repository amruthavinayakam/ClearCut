...
export function HumanDisposition({ projectId, item, client = dispositionClient }: { projectId: string; item: ClearanceItem; client?: DispositionClient }) {
  ...
  return (
    <section className="mt-5 border-t border-border pt-4">
      <div className="mb-3 flex items-center justify-between"><p className="font-mono text-[9px] tracking-[0.08em] text-muted-foreground">HUMAN DISPOSITION</p><span className="text-[10px] text-muted-foreground">Immutable audit event</span></div>
      <div className="grid grid-cols-2 gap-1.5">{outcomes.map((outcome) => <Button className="justify-start font-normal" key={outcome.status} onClick={() => setSelected(outcome)} size="sm" variant="outline">{outcome.label}</Button>)}</div>
      ...
      <AlertDialog onOpenChange={(open) => { if (!open) setSelected(null); }} open={Boolean(selected)}>
        <AlertDialogContent>
          ...
          <div className="space-y-3">
            <div><Label htmlFor="actor-name">Actor name</Label><Input className="mt-1.5" id="actor-name" onChange={(event) => setActorName(event.target.value)} placeholder={selected?.actor === "counsel" ? "Counsel name" : "Coordinator name"} value={actorName} /></div>
            <div><Label htmlFor="rationale">Rationale</Label><Textarea className="mt-1.5 min-h-24" id="rationale" onChange={(event) => setRationale(event.target.value)} placeholder="State what was reviewed and why this disposition is appropriate." value={rationale} /></div>
            {selected?.status === "counsel_approved" && item.documents.length === 0 && <p className="text-[11px] text-risk-amber">A supporting production document is required before counsel approval can be recorded.</p>}
