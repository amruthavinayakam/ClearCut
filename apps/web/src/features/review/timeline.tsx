...
export function Timeline({ items, duration, selectedId, onSelect }: { items: ClearanceItem[]; duration: number; selectedId: string | null; onSelect: (id: string, representativeTime: number) => void }) {
  return (
    <div className="border-t border-border bg-background px-4 py-3">
      <div className="relative h-8" aria-label="Risk timeline">
        ...
        {items.flatMap((item) => item.cut_detections.slice(0, 1).map((detection) => {
          ...
          return <button aria-label={`${item.name} at ${detection.representative_time.toFixed(1)} seconds`} className={`absolute top-1 size-3 -translate-x-1/2 rounded-full border-2 border-background risk-fill--${item.color} ${selectedId === item.id ? "ring-2 ring-primary/30" : ""}`} key={item.id} onClick={() => onSelect(item.id, detection.representative_time)} style={{ left: `${left}%` }} type="button" />;
        }))}
      </div>
