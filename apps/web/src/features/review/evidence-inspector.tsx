  ...
  return (
    <aside className="min-w-0 border-t border-border bg-background xl:min-h-0 xl:overflow-y-auto xl:border-l xl:border-t-0">
      ...
      <Tabs defaultValue="overview">
        <TabsList className="h-10 w-full justify-start overflow-x-auto border-b border-border px-2" variant="line">
        ...
        <TabsContent className="p-4" value="overview">
          <p className="text-xs leading-5 text-pretty text-muted-foreground">{item.research_summary || item.description}</p>
          ...
          <div className="mt-5 rounded-xl bg-muted/50 p-3.5 shadow-[inset_0_0_0_1px_oklch(0_0_0/0.04)]">
            ...
            <p className="mt-2 text-xs leading-5 text-pretty">{item.recommended_actions[0] ?? "Review the current evidence and record the next coordination step."}</p>
          </div>
          ...
          {item.documents.length === 0
            ? (
              <div className="rounded-xl bg-muted/40 px-4 py-7 text-center shadow-[inset_0_0_0_1px_oklch(0_0_0/0.04)]">
                ...
                <p className="mt-1 text-xs leading-5 text-pretty text-muted-foreground">Attach a licence, release, permit, or correspondence before recording document-dependent outcomes.</p>
              </div>
            )
